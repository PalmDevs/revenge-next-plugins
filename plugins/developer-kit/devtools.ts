	...
	for (const [key, level] of [
		...
		unpatches.push(
			instead(console, key, (args, orig) => {
				if (DTContext.con && DTContext.client) DTContext.client.log(level, args)
...
export function connect() {
	...
	client.expose('hidden', api.unscoped.hidden)
	...
	ws.addEventListener('close', (e_: unknown) => {
		cleanup()
