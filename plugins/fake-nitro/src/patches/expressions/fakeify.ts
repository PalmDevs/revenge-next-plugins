...
function patchEmojiPicker({
	...
	cleanup(
		...
		getModules(withProps<EmojiUtils>('countEmoji'), exports => {
			...
			const [EmojiConstants] = lookupModuleWithImportedPath<EmojiConstants>(
			...
			cleanup(
				instead(
					...
					function (args, orig) {
						...
						if (ALLOWED_INTENTIONS.has(intention) && !bypass) return null
	...
	cleanup(
		getModules(
			...
			MessageActionCreators => {
				...
				const ChannelStore = Stores.ChannelStore as DiscordModules.Flux.Store<{
				...
				async function patchSendOrEditArgs(
					...
					const canEmbedLinks =
						channel &&
						(channel.isPrivate() || PermissionStore.can(EMBED_LINKS, channel))

					let bypassed = false

					function replaceEmoji(emoji: BasicEmoji) {
						const link = buildEmojiUrl(emoji, settings.expressions.emojis.size)

						const text = !settings.expressions.hyperlink
							? `${link}&name=${encodeURIComponent(emoji.name)}`
							: linkWithConfig(link, emoji.name, settings.expressions.hyperlink)

						data.content = data.content.replace(buildEmojiString(emoji), text)
					}
					...
					if (data.validNonShortcutEmojis?.length) {
						...
						data.validNonShortcutEmojis = data.validNonShortcutEmojis.filter(
					...
					instead(
						...
						async function (args, original) {
							...
							const canUseExternalStickers =
								canActuallyUse(ProductCatalog.STICKERS_EVERYWHERE) &&
								(isPrivateChannel ||
									PermissionStore.can(USE_EXTERNAL_STICKERS, channel))

							const canEmbedLinks =
								isPrivateChannel || PermissionStore.can(EMBED_LINKS, channel)
