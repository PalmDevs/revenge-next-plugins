		...
		<TableRowGroup title="Emojis" hasIcons>
			...
			{settings.emojis.size > 0 && (
				<TableRow
					label="Emoji size"
					trailing={
						<TableRow.TrailingText text={`${settings.emojis.size}px`} />
					}
					subLabel={
						<Slider
							...
							startIcon={
								<TableRow.TrailingText text={emojiSizeLadder[0].toString()} />
							}
							endIcon={
							...
							value={
								emojiSizeLadder.indexOf(
									settings.emojis.size as (typeof emojiSizeLadder)[number],
								) ?? 1
							}
						/>
					}
		...
		<TableRowGroup title="Stickers" hasIcons>
			...
			{settings.stickers.size > 0 && (
				<TableRow
					label="Sticker size"
					trailing={
						<TableRow.TrailingText text={`${settings.stickers.size}px`} />
					}
					subLabel={
						<Slider
							...
							minimumValue={0}
							onValueChange={index => {
								const size = stickerSizeLadder[Math.round(index)]
								if (size !== undefined) {
									api.jsonStorage.set({
										expressions: {
											stickers: { size },
							...
							startIcon={
								<TableRow.TrailingText text={stickerSizeLadder[0].toString()} />
							}
							endIcon={
								<TableRow.TrailingText
							...
							value={
								stickerSizeLadder.indexOf(
									settings.stickers.size as (typeof stickerSizeLadder)[number],
								) ?? 3
							}
						/>
					}
